...
namespace V3X.WindowsAPICodePack.Dialogs
{
    ...
    internal static partial class UnsafeNativeMethods
    {
        ...
        internal static extern void TaskDialogIndirect(
            [In] ref TaskDialogConfiguration pTaskConfig,
            [Out] out int pnButton,
        ...
        internal struct TaskDialogConfiguration
        {
            /// <summary>
    ...
    public class TaskDialog
    {
        ...
        /// footer icon will be displayed.
        /// </summary>
        private IntPtr customFooterIcon;
        ...
        /// footer icon will be displayed.
        /// </summary>
        public IntPtr CustomFooterIcon
        {
        ...
        public string DetailsExpandedText
        {
            ...
            set { this.detailsExpandedText = value; }
        }
        ...
        */
        ...
        private int PrivateShow(IntPtr hwndOwner, out bool verificationFlagChecked, out int radioButtonResult)
        {
            ...
            radioButtonResult = 0;
            int result = 0;
            UnsafeNativeMethods.TaskDialogConfiguration config = new UnsafeNativeMethods.TaskDialogConfiguration();

            try
            {
                config.cbSize = (uint)Marshal.SizeOf(typeof(UnsafeNativeMethods.TaskDialogConfiguration));
                ...
                {
                    config.pszWindowTitle = this.caption;
                }

                config.MainIcon = (IntPtr)this.icon;
                if (this.customMainIcon != IntPtr.Zero)
                {
                    config.dwFlags |= UnsafeNativeMethods.TASKDIALOG_FLAGS.TDF_USE_HICON_MAIN;
                    config.MainIcon = this.customMainIcon;
                }

                if (!string.IsNullOrEmpty(this.instructionText))
                {
                    config.pszMainInstruction = this.instructionText;
                }

                if (!string.IsNullOrEmpty(this.text))
                {
                    config.pszContent = this.text;
                }
                ...
                if (!string.IsNullOrEmpty(this.detailsExpandedText))
                {
                    config.pszExpandedInformation = this.detailsExpandedText;
                }
                ...
                if (this.customFooterIcon != IntPtr.Zero)
                {
                    config.dwFlags |= UnsafeNativeMethods.TASKDIALOG_FLAGS.TDF_USE_HICON_FOOTER;
                    config.FooterIcon = this.customFooterIcon;
                }
            ...
            finally
            {
                ...
                if (config.pRadioButtons != IntPtr.Zero)
                {
                    ...
                    for (int i = 0; i < config.cRadioButtons; i++)
                    {
                        unsafe
                        {
                            byte* p = (byte*)config.pRadioButtons + elementSize * i;
                            Marshal.DestroyStructure((IntPtr)p, typeof(TaskDialogButton));
                        }
                    }
